...
class TranslationTrainer() :
    ...
    def my_collate_fn(self, samples):
        ...
        input_ = [sample['input'] for sample in samples]
        ...
        return {
            "input_str":input_str,
            "input":torch.stack(input_).contiguous(),                                              # input
    ...
    def train(self, epochs, train_dataset, eval_dataset, optimizer, scheduler) :
        ...
        for epoch in range(start_epoch, epochs):
            ...
            for i,data in pb:
                ...
                optimizer.zero_grad()
                generator_logit, loss = self.model.forward(input_, target, input_mask, target_mask, labels=target)
    ...
    def evaluate(self, dataset):
        # dropout, batchnorm 등 비활성화
        self.model.eval()
        total_loss = 0.

        self.model.to(self.device)
        
        # autograd engine 비활성화
        with torch.no_grad():
